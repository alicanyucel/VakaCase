...
public sealed class GetByIdDeviceQueryHandler(IDeviceRepository deviceRepository, IDeviceNotifier deviceNotifier) : IRequestHandler<GetByIdDeviceQuery, Result<Device>>
{
    public async Task<Result<Device>> Handle(GetByIdDeviceQuery request, CancellationToken cancellationToken)
    {
        ...
        await deviceNotifier.DeviceViewedAsync(new
        {
            device.Id
        }, cancellationToken);
